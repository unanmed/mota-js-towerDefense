...
declare class defense {
    ...
    /** 取用一个空闲画布，如果不存在空闲画布，则进行创建 */
    acquireCanvas(name: string, type?: string): CanvasRenderingContext2D

    /** 归还画布 */
    returnCanvas(name: string, type?: string): void
    ...
    /** 升级防御塔 */
    upgradeTower(x: number, y: number): void

    /** 自动经验升级 */
    expLevelUp(x: number, y: number): void

    /** 卖出防御塔 */
    sellTower(x: number, y: number): void
    ...
    /** 防御塔经验增加时自动更新状态栏 */
    autoUpdateStatusBar(x: number, y: number): void

    /** 绘制防御塔范围
     * @param square 是否为方形范围
     */
    drawRange(x: number, y: number, range: number, square?: boolean): void

    /** 在状态栏上绘制防御塔的详细信息 */
    drawTowerDetail(ctx: CanvasRenderingContext2D | string, loc: string): void

    /** 在状态栏上绘制建造界面 */
    drawConstructor(ctx: CanvasRenderingContext2D | string): void
...
} & control & events & loader & enemys & items & maps & ui & utils & icons & actions & plugin & defense & towers
