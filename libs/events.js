    ...
    core.status.autoEvents.forEach(function(autoEvent) {
        ...
        if (x == null && y == null) {
            event = [
                ...
                {
                    "type": "function",
                    "function": "function() { core.autoEventExecuting('" + symbol + "', false); }"
                }
            ];
        } else {
            event = [{
                    "type": "function",
                    "function": "function() { core.pushEventLoc(" + x + ", " + y + ", '" + floorId + "' ); }"
                },
                // 用do-while(0)包一层防止break影响事件流
                { "type": "dowhile", "condition": "false", "data": autoEvent.data },
                {
                    "type": "function",
                    "function": "function() { core.popEventLoc(); core.autoEventExecuting('" + symbol + "', false); }"
                }
            ];
        }
...
events.prototype._action_while = function(data, x, y, prefix) {
    if (core.calValue(data.condition, prefix)) {
        ...
        core.unshift(core.status.event.data.list, { "todo": list, "total": core.clone(list), "condition": data.condition });
    }
    core.doAction();
}
...
events.prototype.eventMoveHero = function(steps, time, callback) {
    ...
    var step = 0,
        moveSteps = (steps || []).map(function(t) {
